...
async function setWebhook(token: string, url: string, secretToken: string): Promise<boolean> {
...
async function setMenuButton(token: string, miniAppUrl: string): Promise<boolean> {
...
async function setBotCommands(token: string): Promise<boolean> {
  ...
  console.log(`Set ${commands.length} bot commands for private chats`);

  // Set commands for all group chats
  const groupResult = await telegramApi<boolean>(token, 'setMyCommands', {
    commands,
    scope: { type: 'all_group_chats' },
  });

  if (groupResult) {
    console.log(`Set ${commands.length} bot commands for group chats`);
  }
...
async function setup(config: SetupConfig): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  aws ssm put-parameter --name "/fracti/${stage}/bot/webhook-secret" --value "${secretToken}" --overwrite`,
    );

    // Privacy Mode warning
    if (!botInfo.can_read_all_group_messages) {
      console.log('\n⚠️  IMPORTANT: Bot Privacy Mode is ENABLED');
      console.log('   In groups, the bot will only receive:');
      console.log(`   - Commands with bot username (e.g., /start@${botInfo.username})`);
      console.log('   - Replies to bot messages');
      console.log('   - Messages that @mention the bot');
      console.log('\n   To receive all group messages, disable Privacy Mode:');
      console.log('   1. Open @BotFather');
      console.log('   2. Send /mybots → Select bot → Bot Settings → Group Privacy → Turn off');
    }
  } catch (error) {
    console.error('\nSetup failed:', error);
    process.exit(1);
