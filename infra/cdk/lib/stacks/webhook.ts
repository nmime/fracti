...
export class WebhookStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: WebhookStackProps) {
    ...
    this.secretToken = crypto.createHash('sha256').update(tokenSource).digest('hex').substring(0, 32);

    this.secretTokenMd5 = crypto.createHash('md5').update(this.secretToken).digest('hex');
