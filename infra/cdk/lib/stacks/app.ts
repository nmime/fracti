...
// Re-export modular stacks (deprecated - use FractiApp single stack)
...
export class FractiApp extends Construct {
  ...
  constructor(scope: Construct, id: string, props: FractiAppProps) {
    ...
    const lambdaEnvironment: Record<string, string> = {
      ...
      MINI_APP_URL: process.env.MINI_APP_URL || 'https://t.me/FractiBot/app', // t.me deeplink for URL buttons
      APP_URL: process.env.APP_URL || '', // CloudFront URL for webApp buttons (private chats)
    };
    ...
    // Bedrock permissions - foundation models and inference profiles (all US regions for cross-region inference)
    const bedrockPolicy = new iam.PolicyStatement({
      ...
      resources: [
        ...
        // Inference profiles
        `arn:aws:bedrock:${env.region || 'us-east-1'}:${env.account}:inference-profile/us.anthropic.*`,
        `arn:aws:bedrock:${env.region || 'us-east-1'}:${env.account}:inference-profile/global.anthropic.*`,
      ],
    });
    ...
    botFunction.addToRolePolicy(marketplacePolicy);
    guiFunction.addToRolePolicy(marketplacePolicy);
    apiFunction.addToRolePolicy(marketplacePolicy);
    ...
    // Response Headers Policy for CORS (TON Connect manifest)
    const corsHeadersPolicy = new cloudfront.ResponseHeadersPolicy(mainStack, 'CorsHeadersPolicy', {
      responseHeadersPolicyName: `${prefix}-cors-${stage}`,
      corsBehavior: {
    ...
    const distribution = new cloudfront.Distribution(mainStack, 'Distribution', {
      ...
      additionalBehaviors: {
        // TON Connect manifest with CORS headers
  ...
  constructor(scope: Construct, id: string, props: FractiStackProps) {
    ...
    this.guiFunction = new lambda.Function(this, 'GuiFunction', {
      ...
      code: lambda.Code.fromAsset(path.join(__dirname, '../../../../apps/web'), {
        bundling: {
          image: lambda.Runtime.NODEJS_22_X.bundlingImage,
          command: ['bash', '-c', 'npm install && npm run build && cp -r build/* /asset-output/'],
          user: 'root',
        },
      }),
      environment: {
    ...
    const bedrockPolicy = new iam.PolicyStatement({
      ...
      resources: [
        ...
        `arn:aws:bedrock:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:inference-profile/global.anthropic.*`,
      ],
    });
    ...
    this.botFunction.addToRolePolicy(marketplacePolicy);
    this.guiFunction.addToRolePolicy(marketplacePolicy);
    ...
    // Bot webhook route
    const botIntegration = new apigatewayIntegrations.HttpLambdaIntegration('BotIntegration', this.botFunction);
    this.api.addRoutes({
    ...
    // GUI routes (SSR)
    const guiIntegration = new apigatewayIntegrations.HttpLambdaIntegration('GuiIntegration', this.guiFunction);
    this.api.addRoutes({
    ...
    // API Gateway origin
    const apiOrigin = new cloudfrontOrigins.HttpOrigin(`${this.api.apiId}.execute-api.${cdk.Aws.REGION}.amazonaws.com`);
