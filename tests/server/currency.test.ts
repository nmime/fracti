import { SUPPORTED_CURRENCIES, currencyService, type Currency } from '@server/services/currency.service';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

// Destructure service methods
const { formatCurrency, convertCurrency, getExchangeRate, getAllRates, convertExpenseCurrency, getExchangeRates } =
  currencyService;
    ...
    it('should fetch TON price and fiat rates', async () => {
      vi.mocked(global.fetch)
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ 'the-open-network': { usd: 5.5 } }),
        } as Response)
        .mockResolvedValueOnce({
          ok: true,
          json: () =>
  ...
  describe('convertCurrency', () => {
    beforeEach(() => {
      vi.mocked(global.fetch)
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ 'the-open-network': { usd: 5.0 } }),
        } as Response)
        .mockResolvedValueOnce({
          ok: true,
          json: () =>
