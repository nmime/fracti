...
import { groupIdParamSchema, expenseIdParamSchema, createExpenseSchema, paginationQuerySchema } from '../schemas';
...
routes.get('/:groupId/expenses/:expenseId', requireAuth, zValidator('param', expenseIdParamSchema), async (c) => {
  const user = getCurrentUser(c);
...
routes.delete('/:groupId/expenses/:expenseId', requireAuth, zValidator('param', expenseIdParamSchema), async (c) => {
  const user = getCurrentUser(c);
  const { groupId, expenseId } = c.req.valid('param');
