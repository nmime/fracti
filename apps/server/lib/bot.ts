...
import { getGroup, createGroup, getUser, upsertUser, getGroupMembers, getGroupsByUser, createExpense } from '@libs/db';
...
async function registerUserWithAvatar(groupId: string, user: User): Promise<void> {
  ...
  const memberRecord = await upsertUser(groupId, {
    telegramId,
    name,
  ...
  logger.info('Registered user in group', { groupId, telegramId, memberRecord });
}
...
async function handleExpenseMessage(ctx: Context): Promise<void> {
  ...
  try {
    ...
    if (!currentMember) {
      logger.error('Current user not found in group', { groupId, telegramId: user.id });
      return;
    }

    // Resolve payer - default to current user's member UUID
    let payerId = currentMember.id;
    let payerName = currentMember.name;
    ...
    const splitText = splits.length > 1 ? t('bot.expense.splitWays', { count: splits.length, each: eachAmount }) : '';
...
bot.on('inline_query', async (ctx) => {
  ...
  if (match) {
      ...
      for (const membership of memberships.slice(0, 10)) {
        ...
        results.push({
          ...
          input_message_content: {
            message_text:
              `💸 <b>${t('bot.inline.newExpense')}</b>\n\n` +
              `${t('bot.inline.amount')}: <b>${amount} ${group.currency ?? DefaultCurrency}</b>\n` +
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {
                  text: `✅ ${t('bot.inline.addExpense')}`,
                  web_app: {
                    url: `${MINI_APP_URL}?group=${groupId}&amount=${amount}&desc=${encodeURIComponent(description)}`,
                  },
                },
              ],
            ],
    ...
    results.push({
      ...
      input_message_content: {
        message_text:
          `💸 <b>${t('bot.inline.expenseToSplit')}</b>\n\n` +
    ...
    results.push({
      ...
      input_message_content: {
        message_text:
          `💡 <b>${t('bot.inline.inlineModeTitle')}</b>\n\n` +
