import { getGroup, getGroupMembers, getAllSettlements } from '@libs/db';
import { DefaultCurrency } from '@libs/types';
import { generateExpenseReport, type ExpenseReportRow } from '../services';
...
export function generateExpenseCSV(expenses: ExpenseReportRow[]): string {
  const headers = ['Date', 'Description', 'Amount', 'Currency', 'Paid By', 'Category', 'Split With', 'Your Share'];
...
export async function generateSettlementCSV(groupId: string): Promise<string> {
  ...
  const headers = ['Date', 'From', 'To', 'Amount', 'Currency', 'Status', 'Transaction Hash', 'Completed At'];
 ...
 */
export async function generateGroupReportCSV(groupId: string, startDate?: string, endDate?: string): Promise<string> {
 ...
 */
export async function generateHTMLReport(groupId: string, startDate?: string, endDate?: string): Promise<string> {
  const [group, members, expenses, settlements] = await Promise.all([
    getGroup(groupId),
    getGroupMembers(groupId),
    generateExpenseReport(groupId, undefined, startDate, endDate),
  ...
  const currency = group?.currency ?? DefaultCurrency;
