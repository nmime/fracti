...
import { docClient, TABLE_NAME, getGroup, createExpense } from '@libs/db';
import { DefaultCurrency, type ExpenseSplit } from '@libs/types';
 ...
 */
export async function getRecurringTemplates(groupId: string): Promise<RecurringTemplateRecord[]> {
 ...
 */
export async function deleteRecurringTemplate(groupId: string, templateId: string): Promise<void> {
  await docClient.send(
    new DeleteCommand({
      TableName: TABLE_NAME,
      Key: recurringKeys.template(groupId, templateId),
 ...
 */
export async function getDueTemplates(beforeDate: string): Promise<RecurringTemplateRecord[]> {
  const result = await docClient.send(
    new QueryCommand({
      TableName: TABLE_NAME,
      IndexName: 'GSI1',
...
/**
 * Get due templates for a specific group
 */
export async function getDueTemplatesByGroup(
  groupId: string,
  beforeDate: string = new Date().toISOString(),
): Promise<RecurringTemplateRecord[]> {
  const result = await docClient.send(
    new QueryCommand({
      TableName: TABLE_NAME,
      KeyConditionExpression: 'PK = :pk AND begins_with(SK, :sk)',
      FilterExpression: 'nextRun <= :date AND isActive = :active',
      ExpressionAttributeValues: {
        ':pk': `GROUP#${groupId}`,
        ':sk': 'RECURRING#',
        ':date': beforeDate,
 ...
 */
export async function processRecurringTemplate(template: RecurringTemplateRecord): Promise<void> {
  ...
  try {
    ...
    await notifyNewExpense(
      ...
      template.payerName,
      template.amount,
      template.currency ?? group.currency ?? DefaultCurrency,
    ...
    logger.error('Failed to process recurring template', { templateId: template.id }, error as Error);
    throw error;
  }
}
