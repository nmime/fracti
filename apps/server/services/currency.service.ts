...
export const SUPPORTED_CURRENCIES = ['TON', 'USD', 'EUR', 'GBP', 'RUB', 'UAH', 'USDT', 'BTC', 'ETH'] as const;
...
class CurrencyService {
  ...
  async convertCurrency(amount: number, fromCurrency: Currency, toCurrency: Currency): Promise<number> {
  ...
  async convertExpenseCurrency(
    ...
    return {
      ...
      splits: splits.map((split) => ({
        ...split,
        amount: split.amount * rate,
      })),
    };
  }
}

// Create instance and bind methods to preserve 'this' context when destructured
const service = new CurrencyService();
export const currencyService = {
  getExchangeRates: service.getExchangeRates.bind(service),
  convertCurrency: service.convertCurrency.bind(service),
  getExchangeRate: service.getExchangeRate.bind(service),
  formatCurrency: service.formatCurrency.bind(service),
  getAllRates: service.getAllRates.bind(service),
  convertExpenseCurrency: service.convertExpenseCurrency.bind(service),
};
