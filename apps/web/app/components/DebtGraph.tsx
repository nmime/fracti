...
export function DebtGraph({ data, onNodeClick, width = 350, height = 300 }: DebtGraphProps) {
  ...
  const nodeCanvasObject = useCallback(
    (node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      const nodeColor = isPositive ? '#22c55e' : '#ef4444'
      ...
      // Draw balance
      const balanceText = `${node.balance >= 0 ? '+' : ''}${formatTON(node.balance)}`
  ...
  const linkCanvasObject = useCallback(
    (link: GraphLink, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      ctx.fillText(`${formatTON(link.amount)} TON`, midX, midY - 10 / globalScale)
    },
    []
  )
