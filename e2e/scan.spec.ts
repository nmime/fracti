...
test.describe('Receipt Scan Page', () => {
  ...
  test('should show camera/upload options', async ({ page }) => {
    ...
    const cameraButton = page.locator('button:has-text("Camera"), button:has-text("Scan"), [data-testid="camera-button"]')
  ...
  test('should create expense from scanned receipt', async ({ page }) => {
    ...
    if (await fileInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const createButton = page.locator('button:has-text("Create Expense"), button:has-text("Save"), button:has-text("Add")')
  ...
  test('should support retrying scan', async ({ page }) => {
    ...
    const retryButton = page.locator('button:has-text("Retry"), button:has-text("Try Again"), button:has-text("Scan Again")')
  })
